...
class RabbitMQConsumer {
  ...
  constructor() {
    this.queueName = env.RABBITMQ_EVENT_QUEUE;
  ...
  private async handleMessage(message: QueueMessage): Promise<void> {
    ...
    try {
      ...
      switch (action) {
        case "createEvent":
          response = await eventController.createEvent(requestId, data, headers);
          break;
        case "getMyEvents":
          response = await eventController.getMyEvents(requestId, data, headers);
          break;
        case "getAllEvents":
          response = await eventController.getAllEvents(requestId, data, headers);
          break;
        case "getEventById":
          response = await eventController.getEventById(requestId, data, headers);
          break;
        case "checkEventRegistration":
          response = await eventController.checkEventRegistration(requestId, data, headers);
          break;
        case "eventRegister":
          response = await eventController.eventRegister(requestId, data, headers);
          break;
        case "getEventAttendees":
          response = await eventController.getEventAttendees(requestId, data, headers);
          break;
        default:
          response = {
            requestId,
